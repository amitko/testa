...
tic
inc = irT.assembly.singleTest(30,irtPatams,'abilityScaleValues',th,...
...
resultsTIF = irT.expected.TestInformation(irtPatams(inc,:),th)
...
tic
inc = irT.assembly.multipleTest(3,30,irtPatams,'abilityScaleValues',th,...
                        'targetInfFunctionValuesDN',targetTestInfFunctionDN,...
                        'targetInfFunctionValuesUP',targetTestInfFunctionUP,...
...
resultsTIF = [];
for k = 1:size(inc,2)
    resultsTIF(:,k) = irT.expected.TestInformation(irtPatams(inc(:,k),:),th)
end
