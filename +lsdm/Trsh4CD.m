...
for k = 1:size(cd_attributes,2)-1
    for l = 1:size(cd_attributes,1)
        ...
        if ~isempty(I1)
            [a1,b1] = tools.find_line_pars([th(k) cd_attributes(l,k)],[th(k+1), cd_attributes(l,k+1)]);
            [a2,b2] = tools.find_line_pars([th(k) cd_attributes(I(I1),k)],[th(k+1), cd_attributes(I(I1),k+1)]);
            x = -(( b1 - b2 )./( a1 - a2 ));
            res(l,I(I1)) = x;
        end;
    end;
